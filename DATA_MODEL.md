...
schema v4 是一次断代重构。正式应用只读写 v4，不再解析旧数组、schema v2／v3、完整备份 v1／v2／v3、旧偏好或历史收入。旧数据通过不被正式页面加载的一次性转换工具生成 v4 完整备份。
...
旧键只用于检测是否需要显示升级引导，不解析、不覆盖、不自动删除。用户可以导入转换后的 v4 完整备份，也可以二次确认后保留旧键并开始空白 v4 账本。
...
转换工具位于 `tools/convert-v3-to-v4.mjs`，不被正式页面加载，真实备份不进入 Git。转换规则：
...
转换报告必须给出原始记录数、支出数、排除收入数、总支出、日期范围、未映射分类、未映射成员和无效专项。只有未映射项为 0 且转换前后支出总额一致时才生成可导入文件。缺少获益方的旧记录必须通过 `--unassigned family|wife|husband|son` 明确指定，工具不会覆盖已经存在的输出文件。
