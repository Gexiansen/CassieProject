...
function filterRecords(records,filters,today=todayStr()){
  ...
  return records.filter(record=>{
    const spendingType=getSpendingType(record.spendingType),project=projectForId(record.projectId),beneficiary=BENEFICIARIES[record.beneficiaryId]||'',haystack=`${record.note} ${project?project.name:''} ${beneficiary} ${spendingType?spendingType.name:''}`.toLocaleLowerCase('zh-CN');
...
function calculateBudget(value,records=state.records,decisionData=decisions){
  ...
  records.forEach(record=>{if(record.date.slice(0,7)===value&&isOrdinarySpending(record))spentCents+=record.amountCents;});
...
function calculateCashflow(records=state.records,decisionData=decisions,baseDate=todayStr()){
  const forecast=spendingForecast(records,decisionData.projects,baseDate),keys=forecast.keys,periods=forecast.periods.filter(item=>item.recordCount>0).map(item=>({key:item.key,recordCount:item.recordCount,expenseCents:item.totals.fixed+item.totals.flexible+item.totals.discretionary})),averageExpenseCents=forecast.ready?forecast.normalCents:0;
  ...
  return {keys,periods,sampleCount:forecast.sampleCount,availableCents,averageExpenseCents,activeGoalCount:activeGoals.length,requiredGoalCents,currentBudget,hasDailyBudget,projectBudgetCents,dailyPlanCents,plannedExpenseCents,planBalanceCents};
}
function cashflowObservations(metrics){
  ...
  else if(!metrics.hasDailyBudget&&metrics.sampleCount<3)items.push({warn:true,text:`历史支出不足 3 个完整月份，暂不使用低可信度样本估算。`});
  if(metrics.planBalanceCents!==null)items.push({warn:metrics.planBalanceCents<0,text:`按${metrics.hasDailyBudget?'本月日常预算':'最近三个月中位数'}${metrics.projectBudgetCents?'、当月专项预算':''}${metrics.activeGoalCount?'和目标建议投入':''}估算，计划后${metrics.planBalanceCents<0?'缺口':'结余'} ¥${fmt(Math.abs(metrics.planBalanceCents))}。`});
  return items;
}
...
function renderProjectsPlanning(){
  ...
  let h=`<div class="card"><h3>🧳 专项计划 <span class="sub">生活事件单独归集</span></h3><p class="planning-note">专项支出按固定、弹性、可选和突发属性复盘，不占用日常总预算。</p><div class="budget-actions"><button class="primary" data-action="open-project-form">＋ 新建专项</button></div></div>`;
...
function renderOverview(recs){
  ...
  else if(breakdown.baselineCents>0)insight=`本期基础生活支出为 ¥${fmt(breakdown.baselineCents)}，暂未记录可选消费。`;
  if(budget&&budget.configured&&budget.percent!==null&&budget.percent>=100){warn=true;insight=`日常预算已经超出 ¥${fmt(Math.abs(budget.remainingCents))}，应优先检查可选消费和偏高的弹性必需。`;}
...
function projectReferenceMarkup(type){
  const reference=recentProjectReference(type,decisions.projects,state.records);if(!reference)return '';
  const {project,metrics,topSpendingType}=reference,typeName=PROJECT_TYPES[type].name,budget=project.budgetCents?`¥${fmt(project.budgetCents)}`:'未设置';
  const detail=type==='travel'?`${metrics.people} 人 · 人均 ¥${fmt(metrics.perPersonCents)} · 人均每天 ¥${fmt(metrics.perPersonDayCents)}`:'';
  return `<div class="project-reference"><div class="project-reference-head"><span>最近一次${typeName}参考</span><b>${esc(project.name)}</b></div><div class="project-reference-stats"><div>实际支出<b>¥${fmt(metrics.actualCents)}</b></div><div>当时预算<b>${budget}</b></div><div>记录周期<b>${metrics.days} 天</b></div></div>${topSpendingType?`<p>主要属性：${esc(topSpendingType.name)} ¥${fmt(topSpendingType.amountCents)}${detail?` · ${detail}`:''}</p>`:''}<small>只作历史参考，不会自动填写本次预算。</small></div>`;
}
...
function projectHistoryComparisonMarkup(project){
  const references=projectHistoryReferences(project.type,decisions.projects,state.records,project.id,5);if(!references.length)return '';
  const rows=references.map(reference=>{const item=reference.project,metrics=reference.metrics,budget=item.budgetCents?`预算 ¥${fmt(item.budgetCents)} · ${metrics.percent.toFixed(1)}%`:'未设置预算',main=reference.topSpendingType?`主要属性 ${esc(reference.topSpendingType.name)} ¥${fmt(reference.topSpendingType.amountCents)}`:'暂无属性汇总',extra=item.type==='travel'?`${metrics.people} 人 · 人均 ¥${fmt(metrics.perPersonCents)} · 人均每天 ¥${fmt(metrics.perPersonDayCents)}`:`${metrics.days} 天 · ${metrics.items.length} 笔`;
...
let form={id:null,date:'',spendingType:'flexible',beneficiaryId:'family',projectId:'',projectTouched:false,showMore:false,quickExpanded:false,noSpendArmed:false};
...
function quickSceneVisible(scene){
  const beneficiary=prefs.beneficiaries.find(item=>item.id===scene.beneficiaryId&&item.active);if(!SPENDING_TYPES[scene.spendingType]||!beneficiary)return false;
...
function selectRecordContext(value){
  ...
  form.projectTouched=true;const select=document.getElementById('fProject');if(select)select.value=form.projectId;renderRecordContext();renderQuickChoices();
}
function openRecordForm(id=null,preset=null){
  ...
  form.date=sourceDate;form.projectId=sourceProject?sourceProject.id:autoProject?currentProject.id:'';form.beneficiaryId=sourceBeneficiary?sourceBeneficiary.id:defaultBeneficiary.id;form.spendingType=source&&SPENDING_TYPES[source.spendingType]?source.spendingType:form.spendingType;form.projectTouched=false;form.quickExpanded=false;form.noSpendArmed=false;
  ...
  const h=`<div class="overlay" data-action="close-overlay"><div class="sheet" role="dialog" aria-modal="true" aria-label="${record?'修改记录':'新增记录'}">
    <div class="record-sticky"><div class="sheet-head"><div class="r1"><div><h3>✏️ ${record?'修改支出':'快速记账'}</h3><p style="font-size:12px;color:#94a3b8;margin-top:3px">${record?'修改属性后点击底部保存':'输入金额后，点击支出属性即可完成'}</p></div><div class="record-head-actions">${noSpendAction}<button class="x" data-action="close-modal" aria-label="关闭">✕</button></div></div></div>
    ...
    <div class="sheet-body record-scroll-body">
      ...
      <div class="field"><label>支出属性</label><div class="spending-type-choices" id="spendingTypeChoices"></div></div>
  ...
  renderRecordContext();renderQuickChoices();renderSpendingTypeChoices();refreshNoSpendButton();document.querySelector('#modals .x').focus({preventScroll:true});
}
function renderQuickChoices(){
  const all=quickRecordScenes(state.records).filter(quickSceneVisible),values=form.quickExpanded?all:all.slice(0,QUICK_SCENE_LIMIT);
  renderedQuickScenes=all;
  const field=document.getElementById('quickField');
  if(!values.length){field.style.display='none';return;}
  field.style.display='block';
  document.getElementById('quickPicks').innerHTML=values.map(scene=>{const index=all.indexOf(scene),spendingType=getSpendingType(scene.spendingType),project=projectForId(scene.projectId),beneficiary=BENEFICIARIES[scene.beneficiaryId]||'未标注',title=scene.note||spendingType.name,detail=[spendingType.name,project&&project.name,beneficiary].filter(Boolean).join(' · ');return`<button data-action="${form.id?'select-quick-scene':'save-quick-scene'}" data-value="${index}" title="${esc(title+' · '+detail)}"><b>${esc(title)}</b><span>${esc(detail)}</span></button>`;}).join('');
...
function applyQuickScene(value,save=false){
  ...
  const select=document.getElementById('fProject'),note=document.getElementById('fNote');if(select)select.value=form.projectId;if(note&&!note.value.trim()&&scene.note)note.value=scene.note;
  renderRecordContext();renderSpendingTypeChoices();if(save)doSave();
}
...
function refreshProjectForDate(){
  ...
  select.value=applies?current.id:'';form.projectId=select.value;renderRecordContext();renderQuickChoices();
}
...
function selectSpendingType(value,save=false){if(!SPENDING_TYPES[value])return;form.spendingType=value;renderSpendingTypeChoices();if(save)doSave();}
function readAmount(){
...
function doSave(){
  ...
  if(!SPENDING_TYPES[form.spendingType]){toast('请选择支出属性');return;}
  const record={id:old?old.id:uuid(),date,spendingType:form.spendingType,amountCents,note,projectId,beneficiaryId:form.beneficiaryId,
...
function parseBackup(parsed,name=''){
  if(!parsed||typeof parsed!=='object'||parsed.backupVersion!==BACKUP_VERSION||parsed.schemaVersion!==SCHEMA_VERSION)throw new Error('只支持完整备份 v5；旧备份请先使用一次性转换工具');
  ...
  return {records:result.records,settings,plans,source:{name,format:'完整备份 v5',exportedAt:parsed.exportedAt||''}};
}
...
document.addEventListener('click',event=>{
  ...
  const actions={
    ...
    'select-record-context':()=>selectRecordContext(value),'select-spending-type':()=>selectSpendingType(value,!form.id),'select-quick-scene':()=>applyQuickScene(value),'save-quick-scene':()=>applyQuickScene(value,true),'toggle-quick-scenes':toggleQuickScenes,
